...
/**
 * Handles shared health, hunger, and saturation between all players.
 * Implements the "Soul Link" mechanic where all players share the same vital stats.
 */
public class SharedStatsHandler {
    ...
    /**
     * Resets all shared stats to default values.
    ...
    /**
     * Syncs a player's stats to the current shared values.
     * Used for late joiners and reconnecting players.
     */
    ...
    /**
     * Called when a player's health changes after taking damage.
     * Updates the master health and syncs to all other players with visual feedback.
     * 
     ...
     */
    public static void onPlayerHealthChanged(ServerPlayerEntity damagedPlayer, float newHealth, DamageSource damageSource) {
        if (isSyncing) return;
        ...
        if (!runManager.isTemporaryWorld(playerWorld.getRegistryKey())) return;
        ...
        try {
            ...
            if (sharedHealth < oldHealth) {
                ...
                net.minecraft.text.Text damageNotification = net.minecraft.text.Text.empty()
                    ...
                    .append(net.minecraft.text.Text.literal(" has taken ").formatted(net.minecraft.util.Formatting.GRAY))
                    .append(net.minecraft.text.Text.literal(damageText + " ❤").formatted(net.minecraft.util.Formatting.RED))
                    .append(net.minecraft.text.Text.literal(" damage.").formatted(net.minecraft.util.Formatting.GRAY));
                server.getPlayerManager().broadcast(damageNotification, false);
                
                for (ServerPlayerEntity player : players) {
                    if (player == damagedPlayer) continue;
                    ...
                    // Apply actual damage to trigger all client-side effects (red flash, screen shake, sound)
                ...
                SoulLink.LOGGER.debug("Health synced: {} -> {} (from {})", 
                    oldHealth, sharedHealth, damagedPlayer.getName().getString());
            }
    ...
    /**
     * Called when a player heals (regeneration, eating golden apple, etc.)
     * Updates the master health and syncs to all players.
     */
     ...
     * Updates master values and syncs to all other players.
     */
    public static void onPlayerHungerChanged(ServerPlayerEntity player, int newFoodLevel, float newSaturation) {
        if (isSyncing) return;
    ...
    /**
     * Periodic sync check - ensures all players stay in sync.
    ...
    public static void tickSync(MinecraftServer server) {
        ...
        try {
            for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                ...
                if (!runManager.isTemporaryWorld(playerWorld.getRegistryKey())) continue;
