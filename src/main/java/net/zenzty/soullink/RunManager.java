...
/**
 * Manages the lifecycle of temporary Fantasy worlds, game timer, and game state. Handles world
 * creation, deletion, portal linking, and player teleportation.
 * 
...
public class RunManager {
    ...
    /**
     * Checks if coordinates are in an ocean biome WITHOUT loading the chunk. This is a massive
     * optimization - avoids generating chunks just to check if they're water.
     */
    ...
    /**
     * Checks if a location is suitable for spawning (solid ground, not water/lava). Now optimized
     * to check biome first before loading chunks.
     */
    private BlockPos checkSpawnLocation(ServerWorld world, int x, int z) {
        ...
        if (groundState.isSolidBlock(world, groundPos) && !groundState.isOf(Blocks.WATER)
                ...
                && !standState.isSolidBlock(world, standPos) && !standState.isOf(Blocks.WATER)
                && !standState.isOf(Blocks.LAVA) && !headState.isSolidBlock(world, headPos)
                && !headState.isOf(Blocks.WATER) && !headState.isOf(Blocks.LAVA)) {
    ...
    /**
     * Gets the next search position in the spiral pattern. Returns null if we've exhausted all
     * search positions.
     */
    ...
    /**
     * Processes one step of the generation (checks a few spawn locations per tick). Called during
     * GENERATING_WORLD state.
     */
    ...
    /**
     * Transitions from GENERATING_WORLD to RUNNING. Forceloads chunks around spawn, teleports all
     * players, then deletes old worlds.
     */
    private void transitionToRunning() {
        if (overworldHandle == null || validSpawnPos == null)
            return;
    ...
    private void teleportPlayerToSpawn(ServerPlayerEntity player) {
        ...
        // Teleport to spawn
        player.teleport(tempOverworld, validSpawnPos.getX() + 0.5, validSpawnPos.getY() + 1,
                validSpawnPos.getZ() + 0.5, Set.of(), 0, 0, true);
    ...
    /**
     * Starts a new run - creates temporary worlds and begins spawn search. Players stay where they
     * are until spawn is found, then teleport directly.
     */
    public void startRun() {
        ...
        SoulLink.LOGGER.info("World created with seed: {}, now searching for spawn...",
                currentSeed);
    }
     ...
     * 
     * @param teleportPlayers If true, teleports players to vanilla spawn first. Set to false when
     *        starting a new run (players go to new world).
     */
    ...
    /**
     * Teleports a late-joining player to the current run. If generating, puts them in spectator. If
     * running, teleports to spawn.
     */
    ...
    /**
     * Fully resets a player for a new run. Clears inventory, effects, XP, advancements, and resets
     * health/hunger.
     */
    ...
    /**
     * Resets all advancements for a player. Revokes all criteria from every advancement.
     */
    ...
    private boolean checkForInput() {
        ...
        // Check for horizontal movement (ignore Y to avoid false positives from small terrain
        // adjustments)
    ...
    /**
     * Gets the linked nether world for portal travel from a given world. If in temp overworld,
     * returns temp nether. If in temp nether, returns temp overworld.
     */
