...
public class SoulLink implements ModInitializer {
    ...
    /**
     * Helper to get ServerWorld from a player.
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            ...
            dispatcher.register(CommandManager.literal("start")
                .executes(context -> {
                    ...
                    if (runManager == null) {
                        context.getSource().sendError(RunManager.formatMessage("Run manager not initialized."));
                        return 0;
                    }
            ...
            dispatcher.register(CommandManager.literal("stoprun")
                ...
                .executes(context -> {
                    ...
                    context.getSource().sendFeedback(
                        () -> RunManager.formatMessage("Run stopped."),
                        true
                    );
            ...
            dispatcher.register(CommandManager.literal("runinfo")
                .executes(context -> {
                    ...
                    Text info = Text.empty()
                        ...
                        .append(Text.literal(runManager.getGameState().name()).formatted(Formatting.WHITE))
                        .append(Text.literal(" | Time: ").formatted(Formatting.GRAY))
                        .append(Text.literal(runManager.getFormattedTime()).formatted(Formatting.WHITE))
                        .append(Text.literal(" | Health: ").formatted(Formatting.GRAY))
                        .append(Text.literal(String.format("%.1f", SharedStatsHandler.getSharedHealth())).formatted(Formatting.WHITE))
                        .append(Text.literal(" | Hunger: ").formatted(Formatting.GRAY))
                        .append(Text.literal(String.valueOf(SharedStatsHandler.getSharedHunger())).formatted(Formatting.WHITE));
    ...
    private static final java.util.Map<Integer, java.util.List<Runnable>> delayedTasks = new java.util.concurrent.ConcurrentHashMap<>();
    
    /**
     * Schedule a task to run after a delay in ticks.
     */
    private static void scheduleDelayed(net.minecraft.server.MinecraftServer server, int delayTicks, Runnable task) {
    ...
    private void registerConnectionEvents() {
        ...
        ServerPlayConnectionEvents.DISCONNECT.register((handler, server) -> {
            ...
            if (runManager != null && runManager.isRunActive()) {
                LOGGER.info("Player {} disconnected during active run", player.getName().getString());
            }
        });
    }
    ...
    private void sendWelcomeMessage(ServerPlayerEntity player) {
        // Header line
        player.sendMessage(Text.literal("                                                                      ").formatted(Formatting.DARK_GRAY, Formatting.STRIKETHROUGH), false);
        
        // Title
        player.sendMessage(Text.empty()
            .append(Text.literal("SOUL LINK SPEEDRUN").formatted(Formatting.RED, Formatting.BOLD))
            .append(Text.literal(" - Roguelike Mode").formatted(Formatting.GRAY)), false);
        ...
        player.sendMessage(Text.empty()
            ...
            .append(Text.literal(" - All players share health and hunger.").formatted(Formatting.GRAY)), false);
        
        // Goal info
        player.sendMessage(Text.empty()
            .append(Text.literal("⚔ ").formatted(Formatting.GOLD))
            .append(Text.literal("Goal").formatted(Formatting.WHITE))
            .append(Text.literal(" - Defeat the Ender Dragon together.").formatted(Formatting.GRAY)), false);
        
        // Death info
        player.sendMessage(Text.empty()
            .append(Text.literal("☠ ").formatted(Formatting.DARK_RED))
            .append(Text.literal("Death").formatted(Formatting.WHITE))
            .append(Text.literal(" - If anyone dies, the run ends for all.").formatted(Formatting.GRAY)), false);
    ...
    private void registerEntityEvents() {
        ...
        ServerLivingEntityEvents.ALLOW_DAMAGE.register((entity, source, amount) -> {
            ...
            // This might trigger slightly early due to armor, but better safe than showing death screen
            if (currentHealth - amount <= 0) {
                LOGGER.info("Lethal damage detected for {} ({} damage, {} health) - triggering game over!", 
        ...
        ServerLivingEntityEvents.AFTER_DAMAGE.register((entity, source, baseDamageTaken, damageTaken, blocked) -> {
            ...
            if (player.getHealth() <= 0) {
                LOGGER.warn("Player {} reached 0 health despite ALLOW_DAMAGE check - triggering game over", 
